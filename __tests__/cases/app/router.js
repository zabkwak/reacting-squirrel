...
describe('Router', () => {
    ...
    it('parses the url with parsing parameters', () => {
        const parsed = Router.parseUrl(true);
        expect(parsed).to.have.all.keys(['protocol', 'slashes', 'auth', 'host', 'port', 'hostname', 'hash', 'search', 'query', 'pathname', 'path', 'href', 'params']);
        expect(parsed.protocol).to.be.equal('http:');
        expect(parsed.slashes).to.be.equal(true);
        expect(parsed.auth).to.be.equal(null);
        expect(parsed.host).to.be.equal('localhost:8080');
